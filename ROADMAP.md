...
| 3 | Structs, enums, pattern matching | sum types + `match`; `Option` kills `null` | shapes model | [x] |
| 4 | Error handling | `Result`/`?` replace exceptions | `04-file-reader` | [~] |
...
## Progress log
<!-- newest first -->
- **2026-06-13** — ✅ Module 3 done: `03-shapes` model. Covered structs/enums via `Shape`, `match`, methods with `&self`, `Option<Shape>`, `Some`/`None`, `expect()` in tests, and ownership vs borrowing through `scale(&shape, factor)`. Started Module 4 scaffold: `04-file-reader`, focused on `Result`/`?` replacing recoverable exceptions.
